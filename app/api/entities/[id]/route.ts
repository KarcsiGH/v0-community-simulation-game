...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("*")
      .eq("id", id)
      .maybeSingle() // Use maybeSingle to avoid 406 error when entity doesn't exist
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    dbEntity.updated_at = new Date().toISOString()
    ...
    const { data, error } = await supabase
      ...
      .eq("id", id)
      .select()
      .maybeSingle() // Use maybeSingle to handle case where entity doesn't exist
